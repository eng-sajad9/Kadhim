...
function addEmployee() {
  ...
  db.ref(empKey).set(employees).then(() => {
    // إضافة الموظف فقط إلى أيام الشهر الحالي المعروض وليس كل attendance
    if (!isCurrentMonthSelected()) {
      alert("✅ تم إضافة الموظف بنجاح (لن يظهر في أي جدول حضور)");
    ...
    db.ref("attendance").once("value").then(snapshot => {
      ...
      // أضف الموظف فقط لأيام الشهر الحالي المعروض
      ...
      alert("✅ تم إضافة الموظف بنجاح (لن يظهر في الأشهر السابقة)");
      document.getElementById("newEmployeeName").value = "";
      loadEmployeesTable();
      generateTable();
    });
